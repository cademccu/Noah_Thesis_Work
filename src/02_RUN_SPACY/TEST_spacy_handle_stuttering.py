...
def formatted_output(s):
    ...
    print("-" * (20 * 8 + 8))
    print(f_string.format("text", "lemma", "pos", "tag", "dep", "shape", "is_alpha", "is_stop", "morph"))
    print("-" * (20 * 8 + 8))

    for token in doc:
        #print(f_string.format(token.text, token.lemma_, token.pos_, token.tag_, token.dep_, token.shape_, token.is_alpha, token.is_stop, "morph"))
        print(f_string.format(token.text, token.lemma_, token.pos_, token.tag_, token.dep_, token.shape_, token.is_alpha, token.is_stop, str(token.morph)))
        #print(token.morph)
